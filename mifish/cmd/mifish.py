...
def main():
    ...
    for data_dir in ([args.seq_dir]+data_dir_other_groups):
        if os.path.isdir(data_dir) is False:
            ...
            exit(1)
    if os.path.isdir(f'{args.output_dir}') is False:
        print(f'Error: Directory {args.output_dir} does not exist', file=sys.stderr)
        exit(1)
    if os.path.isdir(f'{args.output_dir}/MiFishResult') is True:
        print(f'Error: {args.output_dir}/MiFishResult has alreay existed. Please delete or move it.', file=sys.stderr)
        exit(1)
    ...
    rm_p_5=args.primer_fwd, rm_p_3=rm_p_3, blast_identity=args.blast_min_identity, \
        current_task='', current_message='', debug=True, simple_result=args.skip_downstream_analysis, \
            workdir=args.output_dir, cpu=args.threads, db=args.db)
    
    os.system(f'rm -rf {args.output_dir}/MiFishResult/data')
